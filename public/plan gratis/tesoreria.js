document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderCashflowChartAndTable = () => {
        ...
        cashflowChartInstance = new Chart(document.getElementById('cashflow-chart').getContext('2d'), { type: 'line', data: { labels: chartData.labels, datasets: [{ label: 'Saldo Proyectado', data: chartData.balances, borderColor: '#2563eb', backgroundColor: 'rgba(37, 99, 235, 0.1)', tension: 0.1, fill: true }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { ticks: { color: textColor }, grid: { color: gridColor } }, x: { ticks: { color: textColor }, grid: { color: gridColor } } }, plugins: { legend: { labels: { color: textColor } } } } });
    };

    const renderCashflowSection = () => {
        document.getElementById('cashflow-section').innerHTML = `
    ...
    dom.mainContent.addEventListener('input', (e) => {
        if (e.target.id === 'search-input') {
            ...
            const filteredData = dataArray.filter(item => {
                if (moduleKey === 'accounts') return item.name.toLowerCase().includes(searchTerm) || item.bank.toLowerCase().includes(searchTerm);
    ...
    const exportCashflowToCSV = () => {
        const headers = ["Fecha", "Descripcion", "Ingresos", "Egresos", "Saldo Acumulado"];
    ...
    const init = () => {
        ...
        dom.themeToggle.addEventListener('click', () => {
            ...
            applyTheme(newTheme);
            render(); // Re-render charts on theme change
        });
        loadData(); 
        recalculateAllBalances(); 
        render(); 
