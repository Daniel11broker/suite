...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkLoginStatus = () => {
        ...
        console.log('Modo de operación Tesorería:', isUserLoggedIn ? 'Base de Datos (Online)' : 'LocalStorage (Offline)');
    };

    const saveLocalData = () => localStorage.setItem('tesoreria_data_v1', JSON.stringify(tesoreriaData));
    
    const loadData = async () => {
        if (isUserLoggedIn) {
            try {
                const accounts = await api.get('/api/accounts') || [];
                const manualTransactions = await api.get('/api/transactions') || [];
                tesoreriaData = { accounts, manualTransactions };
            } catch(e) {
                console.error("Fallo al cargar datos de la API, usando datos por defecto.", e);
                tesoreriaData = JSON.parse(JSON.stringify(defaultData));
            }
        } else {
            const data = localStorage.getItem('tesoreria_data_v1');
            tesoreriaData = data ? JSON.parse(data) : JSON.parse(JSON.stringify(defaultData));
        }
    };

    // --- 3. UTILITIES (Estilo CRM) ---
    ...
    const recalculateAllBalances = () => {
        if (!tesoreriaData.accounts) tesoreriaData.accounts = [];
        if (!tesoreriaData.manualTransactions) tesoreriaData.manualTransactions = [];

        if (tesoreriaData.accounts.length === 0 && !tesoreriaData.accounts.find(acc => acc.name === 'Caja General')) {
            const newCashAccount = { id: Date.now(), name: 'Caja General', bank: 'N/A', type: 'Efectivo', initialBalance: 0, currentBalance: 0 };
             if(!isUserLoggedIn) tesoreriaData.accounts.push(newCashAccount);
        }
    ...
    const render = () => {
        ...
        const module = activeTab ? activeTab.dataset.target.replace('-section', '') : 'dashboard';
        renderModule(module);
    };
    ...
    const renderModule = (moduleKey) => {
        ...
        switch(moduleKey) {
            ...
            case 'accounts': renderTableSection('accounts'); break;
            case 'transactions': renderTableSection('transactions'); break;
        }
        feather.replace();
    };
    ...
    const renderCashflowSection = () => {
        ...
        section.innerHTML = `
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                <h2 class="text-2xl font-bold mb-4">Proyección de Flujo de Caja (30 días)</h2>
    ...
    const renderTableSection = (moduleKey) => {
        const config = moduleConfig[moduleKey];
        const data = moduleKey === 'accounts' ? (tesoreriaData.accounts || []) : (tesoreriaData.manualTransactions || []);
        const section = document.getElementById(`${moduleKey}-section`);
        if(!section) return;
        
        const tableRows = data.map(item => {
            const actions = `
                <button onclick="window.handleEdit('${moduleKey}', ${item.id})" class="p-1 text-blue-500 hover:text-blue-700" title="Editar"><i data-feather="edit-2" class="h-4 w-4"></i></button>
                <button onclick="window.handleDelete('${moduleKey}', ${item.id})" class="p-1 text-red-500 hover:text-red-700" title="Eliminar"><i data-feather="trash-2" class="h-4 w-4"></i></button>`;

            if (moduleKey === 'accounts') {
                return `<tr class="hover:bg-gray-50 dark:hover:bg-gray-700"><td class="px-4 py-3">${item.name}</td><td class="px-4 py-3">${item.bank}</td><td class="px-4 py-3">${item.type}</td><td class="px-4 py-3 font-medium text-right">${formatCurrency(item.currentBalance)}</td><td class="px-4 py-3 text-right">${actions}</td></tr>`;
            } else { // transactions
        ...
        section.innerHTML = `
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                <div class="flex flex-wrap justify-between items-center mb-4 gap-4">
                    ...
                    <button onclick="window.handleEdit('${moduleKey}', null)" class="bg-blue-600 text-white py-2 px-4 rounded-lg flex items-center hover:bg-blue-700 flex-shrink-0">
                        <i data-feather="plus" class="h-5 w-5 mr-2"></i>Agregar
                    </button>
                </div>
                <div class="overflow-x-auto"><table class="w-full table-auto">
                    <thead><tr class="bg-gray-50 dark:bg-gray-700">${config.headers.map(h => `<th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">${h}</th>`).join('')}</tr></thead>
    ...
    dom.formModal.form.addEventListener('submit', async (e) => {
        ...
        try {
            if (isUserLoggedIn) {
                const endpoint = `/api/${currentModule}${editingId ? `/${editingId}` : ''}`;
                ...
                if (editingId) {
                    ...
                    if (index > -1) dataArray[index] = { ...dataArray[index], ...data, id: Number(editingId) };
                } else {
                    data.id = Date.now();
                    dataArray.push(data);
                }
    ...
    window.handleDelete = async (moduleKey, id) => {
        if (confirm('¿Estás seguro de que quieres eliminar este registro?')) {
            try {
                if (isUserLoggedIn) {
                    await api.delete(`/api/${moduleKey}/${id}`);
                } else {
                    let dataArray = moduleKey === 'accounts' ? tesoreriaData.accounts : tesoreriaData.manualTransactions;
    ...
    // --- 7. INITIALIZATION ---
    const init = async (isReload = false) => {
        if (!isReload) {
            initUserSession();
            checkLoginStatus();
            ...
            dom.formModal.el.addEventListener('click', (e) => { 
                if (e.target.closest('#cancel-btn') || e.target.closest('#close-modal-btn') || e.target === dom.formModal.el) {
                    toggleModal(dom.formModal.el, false);
                }
            });
        }
